...
|---|---|---|
| `app.py` | **General population app** — main file for the PYROX deployment | Yes, entry point 1 |
| `app_athletes.py` | **Athlete app** (beginner→elite) — main file for a second deployment | Yes, entry point 2 |
| `pyrox_bridge.py` | Shared PYROX execution + pace→MET conversion, used by BOTH apps | App-layer |
...
3. Main file path: `app.py` for the general app, `app_athletes.py` for the
   athlete app. Both deployments can point at this same repo — one set of
   modules, two front-ends, so a fix reaches both at once.
